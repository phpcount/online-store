...
const getters = {
    freeCategoryProducts(state) {
        return state.categoryProducts.filter(item => !state.busyProductsIds[item.uuid]);
    }
}; 
...
const actions = {
   ...
   async addNewOrderProduct({ state, dispatch, commit }) {
...
const mutations = {
    ...
    setBusyProductsIds(state) {
        ...
        state.busyProductsIds = state.orderProducts.reduce((obj, item) => (obj[item.product.uuid] = item.product.id,obj), {});
    },
